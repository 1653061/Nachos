...
class OpenFile {
  public:
    ...
    OpenFile(int f) { file = f; currentOffset = 0;}
     OpenFile(int f, int typ) { file = f; currentOffset = 0; type = typ;}	// open the file
    ...
    int Write(char *from, int numBytes) {
		if (type == 1){ // kiem tra file co phai read-only
    ...
    void Seek(int position){ // dua con tro den vi tri position
	currentOffset = position;
    }

    int CurrentOffset(){ // lay vi tri hien tai cua con tro
